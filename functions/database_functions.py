...
def fetch_user_info(email):
    data, count = (
        supabase_client.table("User Registration")
        .select("*")
        .eq("email", email)
        .execute()
    )
...
def delete_user_info(email):
    data, count = (
        supabase_client.table("User Registration").delete().eq("email", email).execute()
    )
...
def insert_user_favorites(email, uri):
    ...
    data, count = (
        supabase_client.table("User Favorites").insert(favorite_data).execute()
    )


def delete_user_favorites(email, uri):
    data, count = (
        supabase_client.table("User Favorites")
        .delete()
        .eq("email", email)
        .eq("dish_uri", uri)
        .execute()
